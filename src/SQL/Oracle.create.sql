...
ALTER TABLE "ТчОтчет"
	ADD CONSTRAINT "ТчОтчет_FНоме_7577" FOREIGN KEY ("Номенклатура") REFERENCES "Номенклатура" ("primaryKey");

CREATE INDEX "ТчОтчет_IНоме_8186" on "ТчОтчет" ("Номенклатура");

ALTER TABLE "ТчОтчет"
	ADD CONSTRAINT "ТчОтчет_FЗака_1643" FOREIGN KEY ("ОтчетОЗак_m0") REFERENCES "ЗаказПост" ("primaryKey");

CREATE INDEX "ТчОтчет_IОтче_2593" on "ТчОтчет" ("ОтчетОЗак_m0");

ALTER TABLE "ТчОтчет"
	ADD CONSTRAINT "ТчОтчет_FОтче_2918" FOREIGN KEY ("ОтчетОЗак_m1") REFERENCES "ОтчетОЗак" ("primaryKey");

CREATE INDEX "ТчОтчет_IОтче_2594" on "ТчОтчет" ("ОтчетОЗак_m1");
